...
#include "ToolsCore\ReadStream.h"
#include "ToolsCore\ByteBuffer.h"
...
namespace ImageHelpers
{
	...
	inline HBITMAP LoadGdiplusImage(_U_STRINGorID bitmap, _U_STRINGorID type = (UINT) 0)
	{
	   USES_CONVERSION;
	...
	inline BOOL ReplaceColorDib32(HDC hDC, HBITMAP hBitmap, DWORD dwKey, DWORD dwReplace = 0UL, DWORD dwShowColor = 0xFF000000)
	{
	   ::GdiFlush();
	   BITMAP bm = { 0 };
	...
	inline HFONT GetDefaultShellFont()
	{
	   static CFont s_font;
	   if( s_font.IsNull() ) {
		  CLogFont lf;
	...
	inline Gdiplus::Bitmap* GdipGetPngBitmapFromResource(_U_STRINGorID bitmap, _U_STRINGorID type = (UINT) 0)
	{
	   CResource res;
	   if( !res.Load(type, bitmap) ) return NULL;
	   DWORD dwSize = res.GetSize();
	...
	inline void ClipBitmapImage( CBitmapHandle bmp )
	{
		...
		auto a = ::SetClipboardData(CF_BITMAP, hDDB);
	...
	bool GdipTargaBufferAsPNG(CReadStream& in_reader, CByteBuffer& out_buffer);
}
